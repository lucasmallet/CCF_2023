...
bray_fanga12 <- plot_ordination(physeq = ecm_fanga12_new, ordination = bray_fanga12_ord, type = "samples", color = "zone")+
  ...
  scale_color_manual(values = treat_color_mds, breaks = treat_order)+
  stat_ellipse(type="t", level=0.70)+
...
# plotting 
bxplt_comp <- df_div_relat %>% ggplot(aes(x = factor(zone, levels = order_level), y = richness, fill =  primary_lifestyle))+
...
bxplt_relat <- ecm_abun_df %>% ggplot(aes(x = factor(zone, levels = order_level), y = ecm_abun, fill = zone))+
  geom_boxplot(width = 0.4, color = "black") +
  coord_cartesian(ylim=c(0,1))+
